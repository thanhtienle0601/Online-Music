...
class PlayArtist extends StatefulWidget {
  final String? id;
  final String? name;
  final String? img;
  const PlayArtist({super.key,this.id,this.name,this.img});

  @override
  State<PlayArtist> createState() => PlayArtistState();
}

class PlayArtistState extends State<PlayArtist> {
  SongAPI? songAPI;
  ...
  void initState() {
    songAPI = SongAPI();
    songs = songAPI!.findByArtistId(widget.id!);
    imgUrl = widget.img!;
    currentIndex = 0;
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(utf8.decode(widget.name!.codeUnits).toUpperCase()),
        centerTitle: true,
