...
const Genre = () => {
  ...
  const totalSongs = genres.length;
  ...
  const pageCount = Math.ceil(genres.length / totalSongsPerPage);
  const handlePageClick = (event) => {
    const newOffset = (event.selected * totalSongsPerPage) % genres.length;
    setItemOffset(newOffset);
  };

  const handleDeleteGenreById = (id) => {
    axios
      .delete(`http://localhost:8085/api/genre/delete/${id}`)
      .then((response) => {
        response;
        enqueueSnackbar('Genre was deleted successfully !', {
          variant: 'success',
        });
          ...
          <table className="w-full bg-black rounded-lg">
            ...
            <tbody>
              {currentItems.map((genre, index) => (
                <tr key={genre.id} className="py-2 hover:bg-[#ffffff30] ">
                  <td className="py-2 px-5 rounded-tl-md rounded-bl-md">
                    {genre.id}
                  </td>

                  <td className="py-2 px-5 ">{genre.name}</td>

                  <td className="py-2 px-5 rounded-tr-md rounded-br-md">
                    <div className="flex items-center justify-start gap-x-4">
                      <Link to={`/genre/edit/${genre.id}`}>
                      ...
                      <RiDeleteBin5Line
                        ...
                        onClick={() => {
                          confirm(
                            ...
                            ? handleDeleteGenreById(genre.id)
                            : null;
                          // enqueueSnackbar(
                          //   `Are you sure you want to delete genre: ${genre.name} ?`,
                          //   {
                          //     action,
                          //     persist: true,
                          //   }
